...
检测通过后可以开启功能。首次配对时，在手机页面点击“允许一台新电脑配对”。先填写电脑名称并选择 USB 模式与 ADB 状态（例如 MTP + ADB），再启动配对。请保持数据线连接并运行电脑端后端；接口就绪后有约 60 秒完成配对。名称和配置会随身份记录一起保存，配对成功后应用该配置。已保存列表提供“编辑”窗口，可同时重命名和修改配置，也可随时移除电脑。

后续插线时，手机会自动验证电脑身份。已保存电脑会在认证后退出临时识别通道，自动应用该电脑保存的 USB 模式与 ADB 状态，不再显示 USB 选择窗口；未知电脑、验证超时或失败时仍会显示普通选择窗口。未知电脑无法自行加入信任列表。旧版本保存的电脑若尚未设置配置，会继续显示选择窗口；在列表中编辑并保存后即可自动应用。编辑后的配置在下次识别该电脑时生效。
